...
from app.schemas.user import UserResponse, UserCreate, UserRoleResponse, UserGroupResponse

from .base import admin, admin_create, admin_edit, admin_delete

router = APIRouter(tags=["User"], prefix="/users",  dependencies=[Depends(PermissionChecker([admin]))])
...
@router.delete("/{user_id}/roles", status_code=status.HTTP_200_OK, response_model=UserRoleResponse)
...
@router.post("{user_id}/groups", status_code=status.HTTP_200_OK, response_model=UserGroupResponse)
...
@router.delete("{user_id}/groups", status_code=status.HTTP_200_OK, response_model=UserGroupResponse)
